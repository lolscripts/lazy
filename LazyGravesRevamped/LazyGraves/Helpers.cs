...
namespace LazyGraves
{
    internal class Helpers
    {
        ...
        {
            if (BushWardSlot() == SpellSlot.Unknown || LastWard < Game.Time + 500) return;
            {
                for (var i = 50; i < 400; i += 50)
                {
                    var extended = Player.ServerPosition.Extend(pos, i);
                    if (!NavMesh.GetCollisionFlags(extended).HasFlag(CollisionFlags.Grass) ||
                        NavMesh.GetCollisionFlags(Player.ServerPosition).HasFlag(CollisionFlags.Grass))
                        continue;

                    Player.Spellbook.CastSpell(BushWardSlot(), (Vector3) extended);
                    LastWard = Game.Time;
                }
            }
        }
